...
class AttributionMethod(object):
    ...
    def session_run(self, T, xs):
        ...
        if self.has_multiple_inputs:
            ...
            #修改后
            print("修改")
            feed_dict.update({self.X[0]: xs[0]})
            feed_dict.update({self.X[1][i]: xs[1][i] for i in range(len(xs[1]))})
            feed_dict.update({self.X[2]: xs[2]})
        else:
            feed_dict[self.X] = xs
            print ("喂数据")
            print(xs)
        ...
        #     '/home/beta/data/code/DeepExplain-master/matrix_complexes/model/my-model-10.meta')
        # new_saver.restore(self.session,
        #                   tf.train.latest_checkpoint("/home/beta/data/code/DeepExplain-master/matrix_complexes/model/"))

        return self.session.run(T, feed_dict)
        # init = tf.global_variables_initializer()
        # return self.session.run(init, feed_dict)
...
class GradientBasedMethod(AttributionMethod):
    ...
    def run(self):
        #问题所在,attributions = None 导致不能往下接着运算.
...
class Gradients(GradientBasedMethod):

    def get_symbolic_attribution(self):
        # self.X = tf.sparse_to_dense(self.X.indices, self.X.dense_shape, self.X.values)
        return tf.gradients(self.T, self.X[1])
"""
